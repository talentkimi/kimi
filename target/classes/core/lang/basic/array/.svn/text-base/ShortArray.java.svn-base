...
import core.lang.basic.type.ShortType;

/**
 * A Short Array.
 */
public class ShortArray implements BasicArray {
	 ...
	 * @param value the value.
	 */
	public final void insert(int index, short value) {
		if (index > size) {
			throw new ArrayIndexOutOfBoundsException(index);
		}
		if (index == size) {
	 ...
	 * @return the count.
	 */
	public final int count(short value) {
		int count = 0;
		for (int i = 0; i < size(); i++) {
			if (get(i) == value) {
				count++;
	...
	public final int count(BasicType type) {
		return count(((ShortType) type).get());
	}
	 ...
	 * @return this as an array.
	 */
	public final short[] toArray(boolean match) {
	 ...
	 * @return the array.
	 */
	public final BasicIndexArray arrayIndexOf(short value) {
	 ...
	 * @return the array.
	 */
	public final BasicIndexArray arrayIndexOf(short value, BasicIndexArray array) {
		for (int i = 0; i < size(); i++) {
			if (get(i) == value) {
				array.add(i);
			}
	...
	public final int indexOf(BasicType type) {
		return indexOf(((ShortType) type).get());
	}
	...
	public final BasicIndexArray arrayIndexOf(BasicType type) {
		return arrayIndexOf(((ShortType) type).get());
	}
	 ...
	 */
	public final short getLargestValue(short max) {
		short value = -1;
		for (int i = 0; i < size; i++) {
	 ...
	 * @return the largest value.
	 */
	public final short getLargestValue() {
		return getLargestValue(Short.MAX_VALUE);
	}
